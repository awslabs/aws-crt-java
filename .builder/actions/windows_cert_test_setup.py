...
class WindowsCertTestSetup(Builder.Action):
    ...
    NOTE: The PFX file has to NOT have a password due to how SecureStrings work in Powershell
    ...
    def run(self, env):
        ...
        if (self.env.shell.getenv("AWS_TEST_MQTT311_IOT_CORE_WINDOWS_PFX_CERT_NO_PASS") == None):
            print("Windows Cert Setup: PFX Cert environment variables not set. Skipping...")
            return
        pfx_cert_path = self.env.shell.getenv("AWS_TEST_MQTT311_IOT_CORE_WINDOWS_PFX_CERT_NO_PASS")
        ...
        if (thumbprint == ""):
            print ("Windows Cert Setup: ERROR - could not find certificate thumbprint")
            return

        self.env.shell.setenv("AWS_TEST_MQTT311_IOT_CORE_WINDOWS_CERT_STORE", windows_certificate_folder + "\\" + thumbprint)
