...
def main():
    ...
    #       sdk 12.1
    # Load command 9
    otool_cmd = "otool -l target/cmake-build/lib/osx/{}/libaws-crt-jni.dylib | grep -A3 \'LC_VERSION_MIN_MACOSX\' | grep -E version | tr -s ' ' | cut -f3 -d' '| tr -d ' ".format(arch)
    ...
    if re.match(r'^(aarch64|armv[6-8]|arm64)', arch):
        ...
        otool_cmd = "otool -l target/cmake-build/lib/osx/{}/libaws-crt-jni.dylib | grep -E minos | tr -s ' ' | cut -f3 -d' ' | tr -d ' '| tr -d ' '".format(arch)
