...
public class Http2ClientConnection extends HttpClientConnection {
    ...
    private Http2ClientConnection(long connectionBinding) {
     ...
     *
     * @param settings The array of settings to change. Note: each setting has its boundary.
     *
     * @return When this future completes without exception, the peer has acknowledged the settings and
     ...
     *
     * @return When this future completes without exception, the peer has acknowledged the PING and
     *         future will be completed with the round trip time in nano seconds for the connection.
     */
     ...
     * @param allowMoreStreams If true, new peer-initiated streams will continue
     ...
     * @param debugData Optional debug data to send. Size must not exceed 16KB.
     */
    public void sendGoAway(final Http2ErrorCode Http2ErrorCode, final boolean allowMoreStreams, final byte[] debugData) {
    }

    /**
     * Increment the connection's flow-control window to keep data flowing.
     *
     * If the connection was created with `manualWindowManagement` set true,
     ...
     * data.
     *
     * If `manualWindowManagement` is false, this call will have no effect.
