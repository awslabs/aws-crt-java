...
/**
 * This class wraps aws-c-http to provide the basic HTTP request/response
 * functionality via the AWS Common Runtime.
 *
 * HttpClientConnection represents a single connection to a HTTP service
 * endpoint.
 *
...
public class HttpClientConnection extends CrtResource {
    ...
    public enum ProtocolVersion {
        HTTP_1_0(1), HTTP_1_1(2), HTTP_2(3);
     ...
     * @return The HttpStream that represents this Request/Response Pair. It can be
    ...
    public HttpStream makeRequest(HttpRequestBase request, HttpStreamResponseHandler streamHandler)
        ...
        HttpStream stream = httpClientConnectionMakeRequest(getNativeHandle(), request.marshalForJni(),
    ...
    /**
     * Releases this HttpClientConnection back into the Connection Pool, and allows
     * another Request to acquire this connection.
     */
    @Override
    ...
    /**
     * Shuts down the underlying http connection. Even if this function is called,
     * you still need to properly close the connection as well in order to release
    ...
    private static void onConnectionAcquired(CompletableFuture<HttpClientConnection> acquireFuture,
            long nativeConnectionBinding, int errorCode) {
        if (errorCode != CRT.AWS_CRT_SUCCESS) {
            acquireFuture.completeExceptionally(new HttpException(errorCode));
            return;
        }
        if (ProtocolVersion.getEnumValueFromInteger(
                (int) httpClientConnectionGetVersion(nativeConnectionBinding)) == ProtocolVersion.HTTP_2) {
     ...
     * Native methods
     ******************************************************************************/
    private static native HttpStream httpClientConnectionMakeRequest(long connectionBinding, byte[] marshalledRequest,
