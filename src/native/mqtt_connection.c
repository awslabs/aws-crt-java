...
void JNICALL Java_software_amazon_awssdk_crt_mqtt_MqttClientConnection_mqttClientConnectionConnect(
    ...
    jshort request_response_timeout_ms) {
    (void)jni_class;
    struct mqtt_jni_connection *connection = (struct mqtt_jni_connection *)jni_connection;
    if (!connection) {
    ...
    connect_options.request_response_timeout_ms = request_response_timeout_ms;
    connect_options.publish_timeout_ms = publish_timeout_ms;
