...
public class CrcBenchmark extends CrtTestFixture {
    ...
    public void crcBenchmark() {
        Checksum crcc = new software.amazon.awssdk.crt.checksums.CRC32();
        java.util.zip.CRC32 crcj = new java.util.zip.CRC32();
        Checksum crc32c = new software.amazon.awssdk.crt.checksums.CRC32C();
        System.out.println("crc32");
        int[] chunk_sizes = new int[] { 1 << 22, 1 << 21, 1 << 20, 1 << 19 };
        profile(1 << 22, chunk_sizes, 1000, 1, crcc);
        profile(1 << 22, chunk_sizes, 1000, 1, crc32c);
        profile(1 << 22, chunk_sizes, 1000, 1, crcj);
        assertEquals(1, 1);
    }

    private double[] update_summary(long count, double mean, double M2, double my_min, double my_max,
            double new_value) {
    ...
    private void print_stats(double[] means, double[] variances, double[] mins, double[] maxs, int[] chunk_sizes) {
        for (int i = 0; i < means.length; i++) {
            System.out.println(String.format("chunk size: %d, min: %,.2f, max: %,.2f, mean: %,.2f, variance: %,.2f",
                    chunk_sizes[i], mins[i], maxs[i], means[i], variances[i]));
        }
    }
    ...
    private double[] profile_sequence(byte[] to_hash, int[] chunk_sizes, int iterations_per_sequence,
    ...
    private void profile(int size, int[] chunk_sizes, int num_sequences, int iterations_per_sequence,
            Checksum checksum_fn) {
