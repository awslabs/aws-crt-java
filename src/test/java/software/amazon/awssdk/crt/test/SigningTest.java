...
public class SigningTest extends CrtTestFixture {
    ...
    private static byte[] TEST_SECRET_ACCESS_KEY = "wJalrXUtnFEMI/K7MDENG+bPxRfiCYEXAMPLEKEY"
    ...
    private HttpRequest createSigv4TestSuiteRequest() throws Exception {
        ...
        HttpHeader[] requestHeaders = new HttpHeader[] { new HttpHeader("Host", "example.amazonaws.com") };
    ...
    private HttpRequest createUnsignableRequest(String method, String path) throws Exception {
        ...
        HttpHeader[] requestHeaders = new HttpHeader[] { new HttpHeader("Authorization", "bad") };
    ...
    public void testSigningSuccess() throws Exception {
        try (StaticCredentialsProvider provider = new StaticCredentialsProvider.StaticCredentialsProviderBuilder()
                .withAccessKeyId(TEST_ACCESS_KEY_ID).withSecretAccessKey(TEST_SECRET_ACCESS_KEY).build();) {
    ...
    public void testSigningBasicSigv4Test() throws Exception {
        try (StaticCredentialsProvider provider = new StaticCredentialsProvider.StaticCredentialsProviderBuilder()
            ...
            try (AwsSigningConfig config = new AwsSigningConfig()) {
                ...
                assertTrue(hasHeaderWithValue(signedRequest, "Authorization",
    ...
    public void testSigv4aHeader() throws Exception {
        ...
        try (AwsSigningConfig config = new AwsSigningConfig()) {
            ...
            assertTrue(hasHeaderWithValuePrefix(signedRequest, "Authorization",
    ...
    /*
     * Chunked encoding signing based on
    ...
    private HttpRequest createChunkedTestRequest() throws Exception {
        ...
        HttpHeader[] requestHeaders = new HttpHeader[] { new HttpHeader("Host", uri.getHost()),
                ...
                new HttpHeader("x-amz-decoded-content-length", "66560"), new HttpHeader("Content-Length", "66824") };
    ...
    private static String EXPECTED_CHUNK_REQUEST_AUTHORIZATION_HEADER = "AWS4-HMAC-SHA256 Credential=AKIAIOSFODNN7EXAMPLE/20130524/us-east-1/s3/aws4_request, "
    ...
    private static byte[] EXPECTED_REQUEST_SIGNATURE = "4f232c4386841ef735655705268965c44a0e4690baa4adea153f7db9fa80a0a9"
            .getBytes(StandardCharsets.UTF_8);
    private static byte[] EXPECTED_FIRST_CHUNK_SIGNATURE = "ad80c730a21e5b8d04586a2213dd63b9a0e99e0e2307b0ade35a65485a288648"
            .getBytes(StandardCharsets.UTF_8);
    private static byte[] EXPECTED_SECOND_CHUNK_SIGNATURE = "0055627c9e194cb4542bae2aa5492e3c1575bbb81b612b7d234b86a503ef5497"
            .getBytes(StandardCharsets.UTF_8);
    ...
    public void testChunkedSigv4aSigning() throws Exception {
        ...
        assertTrue(AwsSigningUtils.verifyRawSha256EcdsaSignature(chunk1StringToSign, chunk1Result.get(),
        ...
        assertTrue(AwsSigningUtils.verifyRawSha256EcdsaSignature(chunk2StringToSign, chunk2Result.get(),
                CHUNKED_SIGV4A_TEST_ECC_PUB_X, CHUNKED_SIGV4A_TEST_ECC_PUB_Y));
        ...
        assertTrue(AwsSigningUtils.verifyRawSha256EcdsaSignature(chunk3StringToSign, chunk3Result.get(),
                CHUNKED_SIGV4A_TEST_ECC_PUB_X, CHUNKED_SIGV4A_TEST_ECC_PUB_Y));
    }
    ...
    // CompletableFuture<HttpRequest> result = AwsSigner.signRequest(request,
    // createChunkedRequestSigningConfig());
    // HttpRequest signedRequest = result.get();
    // assertNotNull(signedRequest);

    // assertTrue(hasHeaderWithValue(signedRequest, "Authorization",
    // EXPECTED_CHUNK_REQUEST_AUTHORIZATION_HEADER));
    ...
    // HttpRequestBodyStream chunk1 = createChunk1Stream();
    // CompletableFuture<AwsSigningResult> chunk1Result = AwsSigner.sign(chunk1,
    // signature,
    ...
    // HttpRequestBodyStream chunk2 = createChunk2Stream();
    // CompletableFuture<AwsSigningResult> chunk2Result = AwsSigner.sign(chunk2,
    // signature,
    // createChunkSigningConfig());
    ...
    // CompletableFuture<AwsSigningResult> finalChunkResult =
    // AwsSigner.sign((HttpRequestBodyStream) null, signature,
    // createChunkSigningConfig());
    ...
    // CompletableFuture<AwsSigningResult> result = AwsSigner.sign(request,
    // chunkedRequestSigningConfig);
    // HttpRequest signedRequest = result.get().getSignedRequest();
    // assertNotNull(signedRequest);

    // byte[] requestSignature = result.get().getSignature();
    // assertTrue(AwsSigningUtils.verifySigv4aEcdsaSignature(request,
    ...
    // CompletableFuture<AwsSigningResult> chunk1Result = AwsSigner.sign(chunk1,
    // requestSignature, chunkSigningConfig);

    // byte[] chunk1StringToSign = buildChunkStringToSign(requestSignature,
    // CHUNK1_STS_POST_SIGNATURE);
    // assertTrue(AwsSigningUtils.verifyRawSha256EcdsaSignature(chunk1StringToSign,
    // chunk1Result.get().getSignature(),
    // CHUNKED_SIGV4A_TEST_ECC_PUB_X, CHUNKED_SIGV4A_TEST_ECC_PUB_Y));
    ...
    // byte[] chunk2StringToSign =
    // buildChunkStringToSign(chunk1Result.get().getSignature(),
    // CHUNK2_STS_POST_SIGNATURE);
    // assertTrue(AwsSigningUtils.verifyRawSha256EcdsaSignature(chunk2StringToSign,
    // chunk2Result.get().getSignature(),
    // CHUNKED_SIGV4A_TEST_ECC_PUB_X, CHUNKED_SIGV4A_TEST_ECC_PUB_Y));

    // CompletableFuture<AwsSigningResult> chunk3Result =
    // AwsSigner.sign((HttpRequestBodyStream) null,
    // chunk2Result.get().getSignature(), chunkSigningConfig);

    // byte[] chunk3StringToSign =
    // buildChunkStringToSign(chunk2Result.get().getSignature(),
    // CHUNK3_STS_POST_SIGNATURE);
    // assertTrue(AwsSigningUtils.verifyRawSha256EcdsaSignature(chunk3StringToSign,
    // chunk3Result.get().getSignature(),
    // CHUNKED_SIGV4A_TEST_ECC_PUB_X, CHUNKED_SIGV4A_TEST_ECC_PUB_Y));
